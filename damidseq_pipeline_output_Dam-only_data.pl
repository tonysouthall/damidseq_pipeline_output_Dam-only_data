...
print "\nPlease enter name for gff file\n";
my $gffname = <STDIN>;
chomp $gffname;
...
my %vars = (
	...
	'samtools_path' => '',
	'bedtools_path' => '',
	'gatc_frag_file' => '/media/tsouthal/DATADRIVE1/NGS/dmel_r6.03/Dmel_BDGP6.GATC.gff',
	'kde_plot' => 0,
	'bowtie2_genome_dir' => '/media/tsouthal/DATADRIVE1/NGS/dmel_r6.03/dmel_r6.03',
	'threads' => 7,
	'full_data_files' => 0,
	'qscore1min' => 0.4,
	'qscore1max' => 1.0,
...
sub write_file {
	...
	open (OUT_Dam, '> just_Dam_channel_GATC_bin.gff');
	...
	foreach my $chr (sort keys %data) {
		foreach my $start (sort {$a <=> $b} keys %{ $data{$chr}}) {
			...
			if ($vars{'output_format'} =~ m/^bed/) {
				...
				print OUT join("\t", "chr$chr", '.', $gffname, $start, $end, $score, '.', '.', '.'), "\n";
				print OUT_Dam "chr$chr\t.\t$gffname\t$start\t$end\t$Dam_only\t.\t.\t.\n";
